...
import { ConflictType, CourseType } from "@ta/shared/utils";
...
export default class SimulatedAnnealing {
    ...
    roomCapacityConflictCost = (
        ...
        for (const cls of currClassAllocation) {
            const roomCapacityConflicts = Class.hasRoomCapacityConflict(cls);
    ...
    instructorConflictCost = (
        ...
        for (const cls of currClassAllocation) {
            const arr = currClassAllocation.filter(
                (iterCls) => iterCls !== cls
            );
            const instructorConflicts = Class.hasInstructorBookingConflict(
                cls,
                arr
            );
    ...
    studentGroupConflictCost = (
        ...
        for (const cls of currClassAllocation) {
            ...
            const studentGroupConflicts = Class.hasStudentBookingConflict(
                cls,
                arr,
                this.data
            );
    ...
    roomConflictCost = (
        ...
        for (const cls of currClassAllocation) {
            ...
            const roomConflicts = Class.hasRoomBookingConflict(cls, arr);
    ...
    facultyTravelConflictCost = (
        ...
        let costVal = 0;
        const [currSchedule] =
            Class.getWeekdayWiseSchedule(currClassAllocation);
    ...
    neighbor2 = () => {
        ...
        const copyCurrClassAllocation = cloneDeep(this.currClassAllocation);

        while (!breakCheck) {
            const [randomClass1, randomClass2] = sampleSize(
            ...
            const randomSlotFromClass1 = sample(randomClass1.slots),
                randomSlotFromClass2 = sample(randomClass1.slots);
            let totSlotCredit1 = 0,
                totSlotCredit2 = 0;
            ...
            const [randomSlotFromData1, randomSlotFromData2] = sampleSize(
                this.data.slots,
                2
            );
    ...
    neighbor1 = () => {
        ...
        while (!breakCheck) {
            const [randomClass1] = sampleSize(copyCurrClassAllocation, 1);
            ...
            const randomSlotFromData = sample(this.data.slots);
    ...
    neighbor = () => {
        ...
        while (!breakCheck) {
            ...
            if (
                ...
                continue;
            const randomSlotFromClass1 = sample(randomClass1.slots);
            const randomSlotFromClass2 = sample(randomClass2.slots);
    ...
    execute = () => {
        ...
        while (Math.abs(currTemperature - this.FINAL_TEMPERATURE) >= 0.000001) {
            ...
            const currCost = this.cost(this.currClassAllocation),
